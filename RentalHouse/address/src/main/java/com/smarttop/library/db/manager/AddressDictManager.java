...
import com.smarttop.library.bean.AddressBean;
...
public class AddressDictManager {
     ...
     * @param adress
     */
    public void inserddress(AddressBean.ChangeRecordsBean adress){
        if(adress !=null){
     ...
     * @param list
     */
    public void insertAddress(List<AddressBean.ChangeRecordsBean> list){
        if(list !=null){
            db.beginTransaction();//手动设置开启事务
            try{
                for (AddressBean.ChangeRecordsBean adress:list) {
                    ContentValues values = new ContentValues();
                    values.put(ADDRESS_DICT_FIELD_CODE,adress.code);
                    values.put(TableField.ADDRESS_DICT_FIELD_NAME,adress.name);
    ...
    //更新地址
    public void updateAddressInfo(AddressBean.ChangeRecordsBean adress){
        if(adress !=null){
            db.beginTransaction();//手动设置开启事务
            try{
                ContentValues values = new ContentValues();
     ...
     * @return
     */
    public List<AddressBean.ChangeRecordsBean> getAddressList(){
        List<AddressBean.ChangeRecordsBean> list = new ArrayList<>();
        Cursor cursor = db.rawQuery("select * from "+ TableField.TABLE_ADDRESS_DICT+" order by sort asc", null);
        while (cursor.moveToNext()){
            AddressBean.ChangeRecordsBean adressInfo =  new AddressBean.ChangeRecordsBean();
    ...
    public List<Province> getProvinceList(){
        ...
        while (cursor.moveToNext()){
            ...
            province.id = cursor.getLong(cursor.getColumnIndex(ADDRESS_DICT_FIELD_ID));
    ...
    public List<City> getCityList(Long  provinceId){
        ...
        while (cursor.moveToNext()){
            City city =  new City();
            city.id = cursor.getLong(cursor.getColumnIndex(ADDRESS_DICT_FIELD_ID));
    ...
    public List<County> getCountyList(Long cityId){
        ...
        while (cursor.moveToNext()){
            ...
            county.id = cursor.getLong(cursor.getColumnIndex(ADDRESS_DICT_FIELD_ID));
    ...
    public List<Street> getStreetList(Long countyId){
        ...
        while (cursor.moveToNext()){
            ...
            street.id = cursor.getLong(cursor.getColumnIndex(ADDRESS_DICT_FIELD_ID));
     ...
     * @return
     */
    public int isExist(AddressBean.ChangeRecordsBean bannerInfo){
        int count = 0;
